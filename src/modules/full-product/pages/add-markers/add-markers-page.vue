<template>
  <v-app class="add-markers-page">
    <navigation>
      <v-btn text @click="$router.push({ path: '/full-product/preview' })">
        <span class="mr-2">Preview</span>
      </v-btn>

      <v-badge bordered :value="'soon'" content="soon" light  overlap>
        <v-btn dark outlined disabled @click="$router.push({ path: '/full-product/add-marker' })">
          <span class="mr-2">edit marker description</span>
        </v-btn>
      </v-badge>
    </navigation>
    <v-main class="add-markers-page-MODEL-main">
...
export default {
  ...
  methods: {
    ...mapMutations("full-product-store", ["setGeoJSON"]),
